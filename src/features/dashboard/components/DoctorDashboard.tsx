...
import { Users, ClipboardList, Activity, Stethoscope, CheckCircle2, AlertOctagon } from 'lucide-react';
import { Resident } from '../../../types/index';
import { StatCard } from './StatCard';
import { Card } from '../../../components/ui/Card';
import { RecentAdmissions } from './RecentAdmissions';
import { useToast } from '../../../app/providers';
import { useDiabetesStore } from '../../../stores/diabetesStore';
...
export const DoctorDashboard = ({ residents, onSelectResident }: { residents: Resident[], onSelectResident: (r: Resident) => void }) => {
   ...
   const { fetchAllRecords, records: diabetesRecords } = useDiabetesStore();
   const [completedTasks, setCompletedTasks] = useState<string[]>([]);
   const [tasks, setTasks] = useState<any[]>([]);

   useEffect(() => {
      const currentMonth = new Date().toISOString().slice(0, 7);
      fetchAllRecords(currentMonth);
   }, []);
   ...
   // Filter high glucose records for today/recent
   const highGlucoseAlerts = diabetesRecords.filter(r => {
      const isHigh = (r.morningBeforeMeal && r.morningBeforeMeal > 7.2) ||
         (r.morningAfterMeal && r.morningAfterMeal > 10.0) ||
         (r.lunchBeforeMeal && r.lunchBeforeMeal > 7.2) ||
         (r.lunchAfterMeal && r.lunchAfterMeal > 10.0) ||
         (r.dinnerBeforeMeal && r.dinnerBeforeMeal > 7.2) ||
         (r.dinnerAfterMeal && r.dinnerAfterMeal > 10.0);
      return isHigh;
            ...
            <div className="space-y-6">
               ...
               <Card title="Cảnh báo y tế">
                  <div className="space-y-3">
                     {highGlucoseAlerts.length > 0 ? (
                        highGlucoseAlerts.map(r => {
                           const resident = residents.find(res => res.id === r.residentId);
                           return (
                              <div key={r.id} className="p-3 bg-red-50 text-red-700 rounded-lg text-sm border border-red-100 flex items-start gap-2">
                                 <Activity className="w-4 h-4 mt-0.5 shrink-0" />
                                 <div>
                                    <p className="font-bold">{resident?.name || 'NCT Unknown'}</p>
                                    <p>Đường huyết cao ngày {r.recordDate}</p>
                                 </div>
                              </div>
                           );
                        })
                     ) : (
                        <div className="p-3 bg-green-50 text-green-700 rounded-lg text-sm border border-green-100 flex items-start gap-2">
                           <CheckCircle2 className="w-4 h-4 mt-0.5 shrink-0" />
                           <div>
                              <p className="font-bold">Đường huyết ổn định</p>
                              <p>Không có chỉ số vượt ngưỡng trong tháng này.</p>
                           </div>
                        </div>
                     )}
