...
const VisitModal = ({ user, onClose, onSave }: { user: User, onClose: () => void, onSave: (v: MedicalVisit) => void }) => {
   ...
   return (
      <Modal
         isOpen={true}
         onClose={onClose}
         title="Ghi nhận khám bệnh"
         footer={
            <>
               <button onClick={onClose} className="px-4 py-2 text-slate-500 hover:bg-slate-100 rounded-lg">Hủy</button>
               <button onClick={handleSave} className="px-4 py-2 bg-teal-600 text-white rounded-lg hover:bg-teal-700">Lưu phiếu khám</button>
            </>
         }
      ...
      >
         <div className="space-y-4">
            <div>
               ...
               <input type="text" className="w-full border rounded-lg p-3" value={data.complaint} onChange={e => setData({ ...data, complaint: e.target.value })} />
            </div>
            <div>
               <label className="block text-sm font-medium mb-1">Chẩn đoán sơ bộ</label>
               <textarea className="w-full border rounded-lg p-3" rows={3} value={data.diagnosis} onChange={e => setData({ ...data, diagnosis: e.target.value })}></textarea>
            </div>
            <div>
               <label className="block text-sm font-medium mb-1">Hướng điều trị / Ghi chú</label>
               <textarea className="w-full border rounded-lg p-3" rows={3} value={data.treatment} onChange={e => setData({ ...data, treatment: e.target.value })}></textarea>
            </div>
         </div>
      </Modal>
   );
