...
export const PrintableForm = ({
  ...
  return (
    <div className="fixed inset-0 bg-white z-[9999] overflow-auto">
      ...
      <div className="flex justify-between items-start border-b-2 border-slate-800 pb-4 mb-8 gap-4">
        <div className="flex items-start gap-4">
          <div className="h-16 w-16 shrink-0 overflow-hidden rounded-xl border border-slate-200 bg-white">
            ...
            <div>
              ...
              <p className="text-sm text-slate-600">Địa chỉ: {branding.address}</p>
              <p className="text-sm text-slate-600">Hotline: {branding.phone}</p>
            </div>
          </div>
          <div className="text-right">
            <h2 className="text-xl font-bold uppercase mt-2">{type}</h2>
            {formId && <p className="text-sm text-slate-500 italic">Mã biểu mẫu: {formId}</p>}
